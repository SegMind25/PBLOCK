...
public class PBlockHelper {
    ...
    public static List<String> getBlockedDomainsForVpn() {
        List<String> all = new java.util.ArrayList<>(BLOCKED_DOMAINS);
        all.addAll(BLOCKED_SEARCH_ENGINES);
        return all;
    }

    public static List<String> getSafesearchDomainsForVpn(boolean google) {
        return google ? GOOGLE_DOMAINS : BING_DOMAINS;
    }

    public static String removeBlockSection(String hostsContent) {
        if (hostsContent == null) return "";
        StringBuilder kept = new StringBuilder();
        boolean skipping = false;
        for (String line : hostsContent.split("\n")) {
            String trimmed = line.trim();
            if (!skipping && trimmed.startsWith(BLOCK_START_MARKER)) {
                skipping = true;
                continue;
            }
            if (skipping) {
                if (trimmed.startsWith(BLOCK_END_MARKER)) {
                    skipping = false;
                }
                continue;
            }
    ...
    public static String applyBlockSection(String hostsContent) {
        return removeBlockSection(hostsContent)
            + "\n"
            + generateBlockEntries();
    }
